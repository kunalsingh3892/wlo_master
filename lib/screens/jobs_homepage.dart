...
class _ChangePageState extends State<JobsScreen> {
  ...
  Future _getZoneAccount() async {
    ...
    var response = await http.get(
      new Uri.https(BASE_URL, API_PATH + "/driver-zones"),
      headers: headers,
    );
    if (response.statusCode == 200) {
      ...
      if (mounted) {
        setState(() {
          ...
          _city =
              (json).map<City>((item) => City.fromJson(item)).toList();
  ...
  Future _checkListData(String type) async {
    ...
    if (isEnabled3 == true) {
      ...
      uri = Uri.parse('https://wloapp.crowdrcrm.com/v1/jobs').replace(query: type);
    }
    ...
    if (response.statusCode == 200) {
      ...
      if(data['data'].length!=0){
          ...
          showLocation=LatLng(double.parse(data['data'][0]['customer']['customer_lat']),double.parse(data['data'][0]['customer']['customer_lng']));
          for (int i = 0; i < data['data'].length; i++) {
            if(data['data'][i]['customer']['customer_lat']!=null) {
              cusName.add(
                  data['data'][i]['customer']['customer_businessname'] + ", " +
                      data['data'][i]['customer']['customer_primaryphone']);
              cusInfo.add(
                  data['data'][i]['customer']['customer_address1'] + ", " +
  ...
  Widget _emptyOrders() {
    return Center(
      child: Container(
          child: Text(
            'NO JOBS FOUND!',
            style: TextStyle(fontSize: 20, letterSpacing: 1, color: Color(0xffb5322f)),
          )),
    );
  }
    ...
    return (await showDialog(
      ...
      builder: (context) => new AlertDialog(
        ...
        actions: <Widget>[
          new FlatButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: new Text("No",style: TextStyle(  color: Color(0xffb5322f),),),
          ),
          new FlatButton(
            onPressed: () {
            ...
            child: new Text("Yes",style: TextStyle(  color: Color(0xffb5322f))),
          ),
        ],
      ),
    )) ?? false;
  }
  Widget mapView(List<LatLng> loc, List cusName, List id,List info){
    return view!=true? GoogleMap( //Map widget from google_maps_flutter package
              zoomGesturesEnabled: true, //enable Zoom in, out on map
              initialCameraPosition: CameraPosition( //innital position in map
                target:showLocation!=null? showLocation:LatLng(0.0, 0.0), //initial position
                zoom: 6.0, //initial zoom level
              ),
              markers: getmarkers(loc,cusName,id,info), //markers to show on map
                        ...
                        child: Container(
                          ...
                          child: RaisedButton(
                            ...
                            onPressed: () {
                              Navigator.pop(context);
                              if (text1) {
                                _checkList = _checkListData("postcode=${postCodeController.text}");

                                if(text2){
                                  _checkList = _checkListData("postcode=${postCodeController.text}&customer_businessname=${bNameController.text}");
                                  if(text3){
                                    _checkList = _checkListData("postcode=${postCodeController.text}"
                                ...
                                if(text2){
                                  _checkList = _checkListData("zone_id=${_type1}&customer_businessname=${bNameController.text}");
                                  if(text1){
                                    _checkList = _checkListData("zone_id=${_type1}"
                                        "&postcode=${postCodeController.text}&customer_businessname=${bNameController.text}");

                                  }
                                }
                                else if(text1){
                                  _checkList = _checkListData("customer_businessname=${bNameController.text}&postcode=${postCodeController.text}");
                                  if(text1){
                                    _checkList = _checkListData("postcode=${postCodeController.text}"
  ...
  Set<Marker> getmarkers(List<LatLng> loc, List cusName,List id,List info) {
    for(int i=0;i<loc.length;i++){
      print(loc.length);
      setState(() {
        markers.add(Marker( //add first marker
          markerId: MarkerId(id[i].toString()),
          position: loc[i] ,//position of marker
          draggable: true,
                                          ...
                                          Expanded(
                                            child: Align(
                                              ...
                                              child: BorderedText(
                                                ...
                                                strokeColor: Color(0xfffbefa7),
                                                child: Text(
                                                    response[index]['customer']['customer_businessname'],
                                                    softWrap: true,
                                                    maxLines: 2,
                                                    overflow: TextOverflow.ellipsis,
                                                    style:normalText
                                    ...
                                    Align(
                                      ...
                                      child: Container(
                                        ...
                                        child: Row(children: [
                                          ...
                                          Expanded(
                                            child: Text(
                                                response[index]['customer']['customer_address1']+", "+response[index]['customer']['customer_address2'],
                                                softWrap: true,
                                                maxLines: 1,
                                                overflow: TextOverflow.ellipsis,
                                                style:normalTex2
                                      ...
                                      child: Align(
                                        ...
                                        child: Container(
                                          child: Row(children: [
                                            ...
                                            Expanded(
                                              child: Text(
                                                  response[index]['customer']['customer_accountnumber'],
                                                  softWrap: true,
                                                  maxLines: 1,
                                                  overflow: TextOverflow.ellipsis,
                                                  style:normalTex2
                                  ...
                                  Row(mainAxisSize: MainAxisSize.max, children: <Widget>[
                                    ...
                                    Expanded(
                                      child: Align(
                                        ...
                                        child: InkWell(
                                          onTap: (){
                                            _launchCaller(response[index]['customer']['customer_primaryphone']);
                                          },
                                          child: Container(
                                            child: Row(children: [
                                              Image(
                                              ...
                                              Expanded(
                                                child: Text(
                                                    response[index]['customer']['customer_primaryphone'],
                                                    softWrap: true,
                                                    maxLines: 1,
                                                    overflow: TextOverflow.ellipsis,
                                                    style:normalTex4
              ...
              child: Container(
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                                ...
                                Expanded(
                                  child: ButtonTheme(
                                    ...
                                    child: RaisedButton(
                                      ...
                                      color: isEnabled1 ? Color(0xffb5322f) : Colors.white,
                                      onPressed: () async {
                                        setState(() {
                                          isEnabled1 = true;
                                          isEnabled2 = false;
                                      ...
                                      child: Text(
                                        "List View",
                                        style: TextStyle(
                                          color: isEnabled1 ? Colors.white : Color(0xffb5322f),
                                          fontSize: 13.0,
                                          fontWeight: FontWeight.w400,
                                        ),
                                      ),
                                    ...
                                    child: RaisedButton(
                                      ...
                                      shape: RoundedRectangleBorder(
                                          borderRadius: BorderRadius.only(
                                              topRight: Radius.circular(25),
                                              bottomRight: Radius.circular(25))),
                                      color: isEnabled2 ? Color(0xffb5322f) : Colors.white,
                                      onPressed: () async {
                                        setState(() {
                                          isEnabled2 = true;
                                          isEnabled1 = false;
                                      ...
                                      child: Text(
                                        "Map View",
                                        style: TextStyle(
                                          color: isEnabled2 ? Colors.white : Color(0xffb5322f),
                                          fontSize: 13.0,
                                          fontWeight: FontWeight.w400,
                                        ),
                                      ),
                                      ...
                                      child: Row(
                                        children: [
                                          Image(
                                            ...
                                            height: 17.0,
                                            width: 17.0,
                                            color: isEnabled3 ? Colors.white : Color(0xffb5322f),
                                          ),
                                          SizedBox(
                                            width: 5.0,
                                          ),
                                    ...
                                    child: RaisedButton(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          "Filter",
                                          style: TextStyle(
                                            color: isEnabled4 ? Colors.white : Color(0xffb5322f),
                                            fontSize: 13.0,
                                            fontWeight: FontWeight.w400,
                                          ),
                                        ),
                      ...
                      Expanded(child: Container(child: isEnabled1?quizList():mapView(loc,cusName,cusId,cusInfo))),
                      SizedBox(
                        height: 10.0,
                      ),
                      Row(children: <Widget>[
                            ...
                            child: Container(
                              ...
                              child: RaisedButton(
                                ...
                                onPressed: ()  {
                                  ...
                                  Navigator.pushNamed(context, '/customer-pagination');
                                },
                                child: Text(
                                  "Add Job",
                                  softWrap: true,
