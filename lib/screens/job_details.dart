...
class _ChangePageState extends State<JobsDetails> {
  ...
  Future _checkListData() async {
    ...
    var response = await http.get(
      new Uri.https(BASE_URL, API_PATH + "/job/$job_id"),
      headers: headers,
    );
    var data = json.decode(response.body);
    print(data);
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        customer_accounttype=data['data']['customer']['customer_accounttype'].toString();
        if(data['data']['customer']['customer_primarycontactemail']!=null) {
          customer_contactemail =
              data['data']['customer']['customer_primarycontactemail']
                  .toString();
        }
        else{
          customer_contactemail ="";
        ...
        if(data['data']['customer']['customer_accountemail']!=null){
          customer_accountemail=data['data']['customer']['customer_accountemail'].toString();
        }
        else{
       ...
       amountController = TextEditingController(text:data['data']['job_amount'] );
      ...
      selectedRegion = data['data']['product']['product_name']+" - "+ data['data']['product']['product_ewc'].toString();
    ...
    return Expanded(
      child: Container(
        ...
        child:Container(
            ...
            child: Html(data:html_currency_code==null?"":html_currency_code+" "+vat.toStringAsFixed(2))),
      )
    );
  }
                          ...
                          Expanded(
                            child: Text(
                                response['customer']['customer_address1'] +
                                    ", " +
                                    response['customer']['customer_address2']+", "+response['customer']['postcode'],
                                softWrap: true,
                                maxLines: 2,
                                overflow: TextOverflow.ellipsis,
                                style: normalTex2),
