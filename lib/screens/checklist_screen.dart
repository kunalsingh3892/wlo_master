...
class _ChangePageState extends State<VehicleChecklists> {
  ...
  Widget CustomDialog({String title, String description, String applyButtonText}){
  ...
  dialogContent(BuildContext context, String title,String description, String applyButtonText) {
    return Stack(
      children: <Widget>[
        Container(
  ...
  Future _checkListData() async {
    ...
    var response = await http.get(
      new Uri.https(BASE_URL, API_PATH + "/checklists"),
      headers: headers,
    );
    ...
    return Scaffold(
      ...
      /*  drawer: Drawer(
        ...
        appBar: AppBar(
          ...
          actions: <Widget>[
            IconButton(
              icon: const Icon(Icons.power_settings_new,color: Color(0xffb5322f),),
              onPressed: () async {
                _logoutPop();
              },
            ),
                            ...
                            onPressed: () async {
                              if (valuefirst) {
                                ...
                                if(proceed){
                                  if(proceed2!=true) {
                                    for (int i = 0;
                                    i < dataResponse['data'].length; i++) {
                                      XMLJSON xmljson = new XMLJSON();
                                      xmljson.checklist_id =
                                      dataResponse['data'][i]['checklist_id'];
                                      xmljson.status =
                                      arr[i] == 1 ? 'Pass' : 'Fail';
                                      xmljson.failed_reason =
                                      arr[i] == 2 ? _controllers[i].text : null;
                                    ...
                                    Fluttertoast.showToast(
                                        msg: "Please fill fail reason", toastLength: Toast
                                        .LENGTH_SHORT);
                                  }
                              }
                                  ...
                                  Fluttertoast.showToast(
                                      msg: "Please select all checklist", toastLength: Toast
                                      .LENGTH_SHORT);
                                }
                              }
                            },
